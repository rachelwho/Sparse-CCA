...
def generate_data(n,d):
    ...
    return Sighat_X, Sighat_Y, Sighat_XY, Ahat, Bhat, vx, vy
...
def MALA_t(A, B, sigmasq, tau,  rho_1, theta_in, t=1):
    theta = theta_in.copy()
...
def Simu_t(A, B, vx, vy, p, n, temp=[1], sigmasq=1, initial = False, Niter = 3000, update = True, loggam = -5, rho_0 = 10, batch = False):
 #     np.random.seed(seed)
    if initial:
...
#     I0 = random.sample(list(np.arange(ndim)),1)[0]
    ...
    norm1 = norm(thetaout[:p//2])
    norm2 = norm(thetaout[p//2:])
    error[0, 2] = min((norm(thetaout[:p//2] / norm1 - vx[:,0])**2), (norm(thetaout[:p//2]/ norm1 + vx[:,0])**2))
    error[0, 3] = min((norm(thetaout[p//2:] / norm2 - vy[:,0])**2), (norm(thetaout[p//2:] / norm2+ vy[:,0])**2))
    ...
    #         print(thetaout[:p//2].dot(B[:p//2,:p//2]).dot(thetaout[:p//2]))
        ...
        norm1 = norm(thetaout[:p//2])
        norm2 = norm(thetaout[p//2:])
        error[jj+1, 2] = min((norm(thetaout[:p//2] / norm1 - vx[:,0])**2), (norm(thetaout[:p//2]/ norm1 + vx[:,0])**2))
        error[jj+1, 3] = min((norm(thetaout[p//2:] / norm2 - vy[:,0])**2), (norm(thetaout[p//2:] / norm2+ vy[:,0])**2))    #         print(thetaout.T.dot(A).dot(thetaout) / n / 2/ (thetaout.dot(B).dot(thetaout)))
...
def ST_mix(A, B, vx, vy, p, n, lag = 1, temp=[1], sigmasq=1, initial = False, Niter = 3000, update = True, loggam = -5, rho_0 = 10, batch = False):
...
#    print(delta0)
    ...
    Simu_t(A, B, vx, vy, p, n, temp=t, sigmasq=sigmasq,initial = (theta0[0,:], delta0[0,:]),Niter = lag, update = update, rho_0 = rho_0, loggam = loggam, batch = batch)
